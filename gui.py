...
def update_filter(sender, app_data):
    pass
        ...
        with dpg.window(tag='w_main'):
            with dpg.tab_bar():
                with dpg.tab(label='Visuals'):
                    ...
                    dpg.add_checkbox(label='Search by Player', tag='c_search_player')
                    dpg.add_input_text(label='Player Name', tag='c_player_name', callback=update_filter)
                    dpg.add_checkbox(label='Filter by Health', tag='c_filter_health')
                    with dpg.group(horizontal=True):
                        dpg.add_radio_button(tag="c_health_condition", items=['Above', 'Below'], default_value="Above", horizontal=True)
                        dpg.add_input_int(label='Health Value', tag='c_health_value', default_value=50, min_value=0, max_value=100)
                ...
                with dpg.tab(label='Config'):
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_slider_int(tag="c_maxdistance", min_value=0, max_value=1000, default_value=0)
                ...
                #     dpg.add_input_text(label='Offset', tag='vm_offset', default_value='4BA500')
