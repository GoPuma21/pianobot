...
def main():
    ...
    while pm.overlay_loop():
        ...
        if dpg.get_value('c_hud'):
            ...
            pm.draw_text(f"Spotify | {pm.get_fps()} fps", 12, 11, 24, Colors.hud)
                                    ...
                                    if not dpg.get_value('c_onlyfriends') or ent.steamid in friends:
                                        if dpg.get_value('c_maxdistance') != 0:
                                            ...
                                            if int(distance / 32) > dpg.get_value('c_maxdistance') and ent.steamid not in friends:
                                                continue
                                        if dpg.get_value('c_filter_health'):
                                            condition = dpg.get_value('c_health_condition')
                                            value = dpg.get_value('c_health_value')
                                            if condition == 'Above':
                                                if ent.health <= value:
                                                    continue
                                            else:  # Below
                                                if ent.health >= value:
                                                    continue
                                        if dpg.get_value('c_search_player') and dpg.get_value('c_player_name'):
                                            search_name = dpg.get_value('c_player_name').lower()
                                            if search_name not in ent.name.lower():
                                                continue
